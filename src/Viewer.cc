...
void Viewer::Run()
{
    ...
    while(1)
    {
        ...
        // Yolo
        {
            std::unique_lock<std::mutex> lock(mMutexPAFinsh); // 加锁，确保线程安全
            for (auto vit = mmDetectMap.begin(); vit != mmDetectMap.end(); vit++) // 遍历检测到的目标
            {
                if (vit->second.size() != 0) // 如果检测到的目标不为空
                {
                    for (auto area : vit->second) // 遍历每个检测到的区域
                    {
                        // 调整识别框位置，将坐标除以Camera.imageScale
                        ...
                        cv::rectangle(toShow, adjustedArea, cv::Scalar(255, 0, 0), 2); // 在图像上绘制矩形框
                        cv::putText(toShow, // 在图像上绘制文本
                                    vit->first, // 文本内容为目标类别
                                    cv::Point(adjustedArea.x, adjustedArea.y), // 文本位置为矩形框的左上角
                                    cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 0), 2); // 字体、大小、颜色和粗细
                    }
                }
